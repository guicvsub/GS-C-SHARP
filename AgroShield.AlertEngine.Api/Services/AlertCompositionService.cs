...
public class AlertCompositionService : IAlertCompositionService
{
    ...
    public AlertaComposicaoResponse Compor(ComporAlertaRequest request)
    {
        ...
        if (request.Geo != null)
        {
            AvaliarColapsoProdutivo(request, candidatos);
            AvaliarSemCoberturaSatelite(request, candidatos);
    ...
    private static void AvaliarColapsoProdutivo(ComporAlertaRequest request, List<AlertaAvaliado> candidatos)
    {
        var geo = request.Geo!;
        if (geo.NdviMedio < 0.15m && geo.UmidadeRelativa >= 0.90m && request.IrrigacaoAtiva && request.EmCultivo)
        {
            candidatos.Add(new AlertaAvaliado(
                Prioridade: 150,
                Severidade: "CRITICO",
                Codigo: "COLAPSO_PRODUTIVO",
                MensagemParaFala: $"Situacao critica no terreno {request.Nome}! NDVI extremamente baixo com excesso de umidade e irrigacao ativa. Risco de perda total da lavoura.",
                MensagemTecnica: $"NDVI={geo.NdviMedio:F2}; umidade={geo.UmidadeRelativa:F2}; irrigacao ativa",
                AcaoRecomendada: "ACIONAR_EQUIPE_EMERGENCIA"
            ));
        }
    }

    private static void AvaliarSemCoberturaSatelite(ComporAlertaRequest request, List<AlertaAvaliado> candidatos)
    {
        ...
        if (dias > 20)
        {
            candidatos.Add(new AlertaAvaliado(
                Prioridade: 140,
                Severidade: "CRITICO",
                Codigo: "SEM_COBERTURA_SATELITE",
                MensagemParaFala: $"Sem imagem de satelite ha mais de vinte dias no terreno {request.Nome}. Inspecao em campo obrigatoria.",
                MensagemTecnica: $"diasSemImagemSatelite={dias}",
                AcaoRecomendada: "INSPECAO_CAMPO_OBRIGATORIA"
            ));
        }
    }
    ...
    private static void AvaliarIrrigacaoDesnecessaria(ComporAlertaRequest request, List<AlertaAvaliado> candidatos)
    {
        ...
        candidatos.Add(new AlertaAvaliado(
            ...
            MensagemParaFala: "A zona sul apresenta boa umidade. Pode adiar a proxima irrigacao para economizar agua.",
